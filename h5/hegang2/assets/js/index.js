	...
	exports['default'] = {
		...
		data: function data() {
			return {
				...
				fullscreen: true,
				vidoeUrl:null// './assets/video/5M.mp4'
			};
		},
		components: {},
	...
	var zmitiUtil = {
		...
		wxConfig: function wxConfig(title, desc, url) {
			...
			_jquery2["default"].ajax({
				...
				jsonp: "callback",
				jsonpCallback: "jQuery224028201837039984734_1534295743433",
				error: function error() {},
				success: function success(data) {
					// console.log("http://api.zmiti.com/weixin/jssdk.php?type=signature&durl=" + code_durl)
...
/***/ (function(module, exports) {
    ...
    <div class="lt-full zmiti-introduce-main-ui " :class="{'show':show}" :style="{background:'url('+imgs.introduceBg+') no-repeat center center',backgroundSize:'cover'}"  ref='page'>
        ...
        <div class="zmiti-introduce-main">
            <div class="zmiti-introduce-title">
                <img :src="imgs.introTitle" alt="">
                <div class='zmiti-introduce-title-text'>{{texts.zlmIntro}}</div>
            </div>
            <div class="zmiti-introduce-content">
        ...
        <nav class="zmiti-nav">
						<ul>
							<li v-for='(item,index) in indexMenu' @touchstart='tabIndex = index' @touchend='tabIndex = -1' :class="{'active':tabIndex === index}"  v-tap='[entryDetail,item.value]'>{{item.name}}</li>
            </ul>
        </nav>
	...
	module.exports = (/\/\*([\s\S]*?)\*\//g).exec(template.toString())[1].replace(/[\t\r\n]/g, '');// "\r\n\t<transition name='main'>\r\n\t\r\n\t\t<div class=\"lt-full zmiti-introduce-main-ui \" :class=\"{'show':show}\" :style=\"{background:'url('+imgs.introduceBg+') no-repeat center center',backgroundSize:'cover'}\"  ref='page'>\r\n\t\t\t<div class=\"zmiti-logo\">\r\n\t\t\t\t<img :src=\"imgs.logo\" alt=\"\">\r\n\t\t\t</div>\r\n\t\t\t<div class=\"zmiti-introduce-main\">\r\n\t\t\t\t<div class=\"zmiti-introduce-title\">\r\n\t\t\t\t\t<img :src=\"imgs.introTitle\" alt=\"\">\r\n\t\t\t\t\t<div>{{texts.zlmIntro}}</div>\r\n\t\t\t\t</div>\r\n\t\t\t\t<div class=\"zmiti-introduce-content\">\r\n\t\t\t\t\t<h2 class=\"zmiti-introduce-name\">\r\n\t\t\t\t\t\t{{texts.zlmName}}\r\n\t\t\t\t\t</h2>\r\n\t\t\t\t\t<div class=\"zmiti-introduce-wrap\" ref='zmiti-introduce-wrap'>\r\n\t\t\t\t\t\t<div>\r\n\t\t\t\t\t\t\t<div v-for='(content,i) in texts.zlmIntroContent' :key=\"i\">\r\n\t\t\t\t\t\t\t\t{{content}}\r\n\t\t\t\t\t\t\t</div>\r\n\t\t\t\t\t\t</div>\r\n\t\t\t\t\t</div>\r\n\t\t\t\t</div>\r\n\t\t\t</div>\r\n\r\n\t\t\t<nav class=\"zmiti-nav\">\r\n\t\t\t\t<ul>\r\n\t\t\t\t\t<li @touchstart='tabIndex = 0' @touchend='tabIndex = -1' :class=\"{'active':tabIndex === 0}\"  v-tap='[entryDetail,\"aigangjingye\"]'>川藏“航标”</li>\r\n\t\t\t\t\t<li @touchstart='tabIndex = 1' @touchend='tabIndex = -1' :class=\"{'active':tabIndex === 1}\" v-tap='[entryDetail,\"yongyuchuangxin\"]'>穿越风雪</li>\r\n\t\t\t\t\t<li @touchstart='tabIndex = 2' @touchend='tabIndex = -1' :class=\"{'active':tabIndex === 2}\" v-tap='[entryDetail,\"ganyufengxian\"]'>流动的绿</li>\r\n\t\t\t\t</ul>\r\n\t\t\t</nav>\r\n\r\n\t\t\t<div class=\"zmiti-team-entry\" v-tap='[showTeamPage]'>制作团队</div>\r\n\r\n\t\t\t<div class=\"zmiti-mask\" v-if='showMasks' @touchstart='showMasks = false'>\r\n\t\t\t\t<img :src=\"imgs.arrow\">\r\n\t\t\t</div>\r\n\t\t\t<div>\r\n\t\t\t\t<Team :obserable='obserable'></Team>\r\n\t\t\t</div>\r\n\t\t</div>\r\n\t\r\n\t</transition>\r\n";
...
/*<transition name='main'>
    <div class="lt-full zmiti-detail-main-ui " :class="{'show':show}" ref='page'>
        <div :style="{background:'url('+imgs.detailBg+') no-repeat center top',backgroundSize:'cover'}">
            <div class="zmiti-index-logo"><img :src="imgs.logo" alt=""></div>
            <h1></h1>
            <div class="zmiti-detail-title"><img :src="data.title" alt=""></div>
            <div v-tap='[toggleVideo]' v-if='data.videoPoster' class="zmiti-video-C" :style="{background:showVideo?'transparent':'url('+data.videoPoster+')
                no-repeat center'}"><video :style="{opacity:showVideo?1:0}" :src="data.videoUrl" x5-playsinline=""
                    ref="video" controls x-webkit-airplay="true" webkit-playsinline="true" playsinline="true">
                </video></div>
            <div class="zmiti-video-text">{{data.videoText}}</div>
            <div v-for='(c,i) in data.content' v-if='data.content&&data.content.length'>
                <div class="zmiti-image" v-if='c.img' :key="i"><img :src="c.img" alt=""></div>
                <div class="zmiti-subtitle" v-if="c.img&&c.title" :key="i">{{c.title}}</div>
                <div class="zmiti-image-text" v-if='c.text'>
                    <div>{{c.text}}</div>
								</div>
								<div class="zmiti-image-text" v-else-if='typeof c=="string" '>
                    <div>{{c}}</div>
                </div>
            </div>
